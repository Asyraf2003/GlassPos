...
final class PrimitiveRevisionConcurrencyFeatureTest extends TestCase
{
    ...
    private function forkRevision(string $worker, string $dir, string $noteId, string $actorId, array $payload): int
    {
        $pid = pcntl_fork();
        if ($pid === -1) {
        ...
        try {
            ...
            $receipt = ['success' => $result->isSuccess(), 'message' => $result->message(), 'data' => $result->data()];
        } catch (Throwable $e) {
